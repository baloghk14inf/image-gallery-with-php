...
/**
 * UpdateImage() -egy kép leítrásának módosítása azonosító alapján
 *
 * @param [type] $connection
 * @param [type] $id
 * @param [type] $title
...
function updateImage($connection,$id,$title)
{
    ...
    if ($statment = mysqli_prepare($connection, $query)) {
        mysqli_stmt_bind_param($statment, "si",$title, $id); //bind-hozzákötés "i"-integer  //az 'si' -az string-et és intet jelent, és ennek megfelelően következnek utána a paraméterek
        mysqli_stmt_execute($statment);
      /*  $result = mysqli_stmt_get_result($statment);
        return mysqli_fetch_assoc($result); */
    } else {
